        ...
        <div class="col">
            <section class="card card-featured card-featured-primary">
                ...
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="{{ route('notification.filter') }}" method="post">
                        ...
                        <div class="form-group row pb-3">
                            ...
                            <div class="col-lg-2">
                                ...
                                <a style="float:right;" href="{{ route('notification.index') }}" class="btn btn-sm btn-default mb-1 mt-1 me-1">Bersihkan</a>
                            </div>
                        </div> 
                    </form>
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="center">{{ $row->notification_date }}</td>
                                    <td class="center">{{ $row->notification_time }}</td>
                                    <td>{{ $row->notification_user }}</td>
                                    <td>{{ $row->notification_title }}</td>
                                    <td>{{ $row->notification_description }}</td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                             data-bs-title="Delete" data-id_delete="{{ $row->notification_id }}"><i
                                                class="fas fa-trash-alt"></i> Hapus</button>
                                        <button type="button" OnClick="link_to('<?php echo 'notification/detail/'. $row->notification_id;  ?>');" class="btn btn-sm btn-info"><i class="fas fa-list-alt"></i> Detail</button>
                                    </td>
                                </tr>
                                <?php } ?>
                                <?php } else { ?>
...
@include('notification.confirm_edit')
@include('notification.confirm_delete')
