        ...
        <div class="col">
            <section class="card card-featured card-featured-primary">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="pull-right">
                                <a class="btn btn-sm btn-primary" href="{{ route('dpo.create') }}" > <i class="fas fa-plus"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                </header>
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="{{ route('dpo.filter') }}" method="post">
                        ...
                        <div class="form-group row pb-3">
                            ...
                            <div class="col-lg-4 mb-2">
                                <input type="text" class="form-control" placeholder="Cari berdasarkan nama dpo ..." 
                                    name="q" value="{{ $q }}" autocomplete="off" />
                            </div>
                            <div class="col-lg-10"></div>
                            <div class="col-lg-2">
                                <button style="float:right;" class="btn btn-sm btn-primary mb-1 mt-1 me-1">Cari</button>
                                <a style="float:right;" href="{{ route('dpo.index') }}" class="btn btn-sm btn-default mb-1 mt-1 me-1">Bersihkan</a>
                            </div>
                        </div> 
                    </form>
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $row->dpo_satker}}</td>
                                    <td>{{ $row->dpo_name}}</td>
                                    <td class="center">
                                        <?php if($row->dpo_image != "") { ?>
                                            <img loading="lazy" src="{{ $row->dpo_path }}" alt="Webphada" class="img-thumbnail" />
                                        <?php } else { ?>
                                            <img loading="lazy" src="{{ asset('assets/img/logo-webphada.png') }}" alt="Webphada" class=" user-image img-thumbnail" />
                                        <?php } ?>   
                                    </td>
                                    <td class="center"><span class="badge badge-<?php echo (($row->dpo_status == 1 ? "info" : "dark")); ?>"><?php echo (($row->dpo_status == 1 ? Status::tipeStatus(1) : Status::tipeStatus(2))); ?></span></td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-warning modalConfirmEdit"
                                            data-bs-toggle="modal" data-bs-title="Edit" data-id_edit="{{ $row->dpo_id }}"><i
                                                class="fas fa-pencil-alt"></i> Ubah</button>
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                            data-bs-toggle="modal" data-bs-title="Delete" data-id_delete="{{ $row->dpo_id }}"><i
...
@include('information.dpo.confirm_edit')
@include('information.dpo.confirm_delete')
