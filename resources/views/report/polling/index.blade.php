        ...
        <div class="col">
            <section class="card card-featured card-featured-primary">
                ...
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="{{ route('polling.filter') }}" method="post">
                        ...
                        <div class="form-group row pb-1">
                            ...
                            <div class="col-lg-4 mb-2">
                                <input type="text" class="form-control" placeholder="Cari berdasarkan alamat ip ..." 
                                    name="q" value="{{ $q }}" autocomplete="off" />
                            </div>
                            <div class="col-lg-10"></div>
                            <div class="col-lg-2">
                                <button style="float:right;" class="btn btn-sm btn-primary mb-1 mt-1 me-1">Cari</button>
                                <a style="float:right;" href="{{ route('polling.index') }}" class="btn btn-sm btn-default mb-1 mt-1 me-1">Bersihkan</a>
                            </div>
                        </div> 
                    </form>
                    <form id="frmUserExport" action="{{route('polling.excell')}}" method="post">
                     <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="center">{{ $row->rating_date }}</td>
                                    <td class="center">{{ $row->rating_time }}</td>
                                    <td>{{ $row->rating_satker}}</td>
                                    <td class="center">{{ $row->rating_value }}</td>
                                    <td>{{ $row->rating_ip }}</td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                             data-bs-title="Delete" data-id_delete="{{ $row->rating_id }}"><i
                                                class="fas fa-trash-alt"></i> Hapus</button>
                                        <button type="button" OnClick="link_to('<?php echo 'report/polling/detail/'. $row->rating_id;  ?>');" class="btn btn-sm btn-info"><i class="fas fa-list-alt"></i> Detail</button>
                                    </td>
                                </tr>
                                <?php } ?>
                                <?php } else { ?>
...
@include('report.polling.confirm_delete')
