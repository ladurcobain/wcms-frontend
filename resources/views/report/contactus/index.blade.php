        ...
        <div class="col">
            <section class="card card-featured card-featured-primary">
                ...
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="{{ route('contact-us.filter') }}" method="post">
                        ...
                        <div class="form-group row pb-1">
                            ...
                            <div class="col-lg-2">
                                ...
                                <a style="float:right;" href="{{ route('contact-us.index') }}" class="btn btn-sm btn-default mb-1 mt-1 me-1">Bersihkan</a>
                            </div>
                        </div> 
                    </form>
                    <form id="frmUserExport" action="{{route('contact-us.excell')}}" method="post">
                     <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="center">{{ $row->contactus_date }}</td>
                                    <td class="center">{{ $row->contactus_time }}</td>
                                    <td>{{ $row->contactus_satker}}</td>
                                    <td>{{ $row->contactus_name }}</td>
                                    <td>{{ $row->contactus_subject }}</td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                             data-bs-title="Delete" data-id_delete="{{ $row->contactus_id }}"><i
                                                class="fas fa-trash-alt"></i> Hapus</button>
                                        <button type="button" OnClick="link_to('<?php echo 'report/contact-us/detail/'. $row->contactus_id;  ?>');" class="btn btn-sm btn-info"><i class="fas fa-list-alt"></i> Detail</button>
                                    </td>
                                </tr>
                                <?php } ?>
                                <?php } else { ?>
...
@include('report.contactus.confirm_delete')
