        ...
        <div class="col">
            <section class="card card-featured card-featured-primary">
                ...
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="{{ route('daily.filter') }}" method="post">
                        ...
                        <div class="form-group row pb-1">
                            ...
                            <div class="col-lg-2 mb-2">
                                <select data-plugin-selectTwo class="form-control populate placeholder" name="month"
                                    data-plugin-options='{ "placeholder": "Pilih Bulan ...", "allowClear": false }'>
                                    <option><option>
                                    <?php for($i=1; $i<=12; $i++) { ?>
                                    <?php $idx = (($i<10)?"0".$i:$i); ?>
                                    <option value="<?php echo $idx; ?>" <?php echo (($month == $idx)?'selected="selected"':''); ?>><?php echo Status::monthName($idx); ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="col-lg-2 mb-2">
                                <select data-plugin-selectTwo class="form-control populate placeholder" name="day"
                                    data-plugin-options='{ "placeholder": "Pilih Tanggal ...", "allowClear": false }'>
                                    <option><option>
                                    <?php for($i=$startOfMonth; $i<=$endOfMonth; $i++) { ?>
                                    <?php 
                                        if($i != 1) {
                                            $idxs = (($i<10)?"0".$i:$i); 
                                        }
                                    ...
                                    ?>
                                    <option value="<?php echo $idxs; ?>" <?php echo (($day == $idxs)?'selected="selected"':''); ?>><?php echo $idxs; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            ...
                            <div class="col-lg-2">
                                ...
                                <a style="float:right;" href="{{ route('daily.index') }}" class="btn btn-sm btn-default mb-1 mt-1 me-1">Bersihkan</a>
                            </div>
                        </div> 
                    </form>
                    <form id="frmUserExport" action="{{route('daily.excell')}}" method="post">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}" />
