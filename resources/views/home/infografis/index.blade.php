                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $row->infografis_satker}}</td>
                                    <td>{{ $row->infografis_name}}</td>
                                    <td class="center">
                                        <?php if($row->infografis_image != "") { ?>
                                            <img loading="lazy" src="{{ $row->infografis_path }}" alt="Webphada" class="img-thumbnail" />
                                        <?php } else { ?>
                                            <img loading="lazy" src="{{ asset('assets/img/logo-webphada.png') }}" alt="Webphada" class=" user-image img-thumbnail" />
                                        <?php } ?>   
                                    </td>
                                    <td class="center"><span class="badge badge-<?php echo (($row->infografis_status == 1 ? "info" : "dark")); ?>"><?php echo (($row->infografis_status == 1 ? Status::tipeStatus(1) : Status::tipeStatus(2))); ?></span></td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-warning modalConfirmEdit"
                                            data-bs-toggle="modal" data-bs-title="Edit" data-id_edit="{{ $row->infografis_id }}"><i
                                                class="fas fa-pencil-alt"></i> Ubah</button>
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                            data-bs-toggle="modal" data-bs-title="Delete" data-id_delete="{{ $row->infografis_id }}"><i
...
@include('home.infografis.confirm_edit')
@include('home.infografis.confirm_delete')
