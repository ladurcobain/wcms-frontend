                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $row->related_satker}}</td>
                                    <td>{{ $row->related_name}}</td>
                                    <td><a target="_blank" href="{{ $row->related_link}}">{{ $row->related_link}}</a></td>
                                    <td class="center"><span class="badge badge-<?php echo (($row->related_status == 1 ? "info" : "dark")); ?>"><?php echo (($row->related_status == 1 ? Status::tipeStatus(1) : Status::tipeStatus(2))); ?></span></td>
                                    <td class="center">
                                        <button type="button" class="btn btn-sm btn-warning modalConfirmEdit"
                                            data-bs-toggle="modal" data-bs-title="Edit" data-id_edit="{{ $row->related_id }}"><i
                                                class="fas fa-pencil-alt"></i> Ubah</button>
                                        <button type="button" class="btn btn-sm btn-danger modalConfirmDelete"
                                            data-bs-toggle="modal" data-bs-title="Delete" data-id_delete="{{ $row->related_id }}"><i
...
@include('home.related.confirm_edit')
@include('home.related.confirm_delete')
