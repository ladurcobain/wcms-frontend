...
use App\Exports\ReportNewsletterExcell;

class ReportNewsletterController extends Controller
{
    private $title = "Laporan";
    private $subtitle = "Kontak Berlangganan";
    private $path = 'report/newsletter/search';
    ...
    public function index() {
        ...
        return redirect()->route('newsletter.search');
	}
    ...
    public function search()
    {
        ...
        return view('report.newsletter.index', $data, compact('results'));
    }
    ...
    {
        $uri = Curl::endpoint();
        $url = $uri .'/'.'newsletter/delete-data';
    ...
    public function detail($id)
    {
        ...
        if($res->status == true) {
            ...
            return view('report.newsletter.detail', $data);
        }
        else {
            Session::flash('alrt', 'error');    
            Session::flash('msgs', $res->message);  
    ...
    {
        return Excel::download(new ReportNewsletterExcell($request->_q, $request->_satker, $request->_start, $request->_end), 'laporan-kontak-berlangganan.xlsx');
    }
